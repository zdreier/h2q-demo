...
const DATA = {
  ...
  allotment: {
    annualGal: 54000,
    usedGal: 26530,
    periodLabel: "2026 Water Year (Oct '25 – Sep '26)",
    monthsRemaining: 6,
  },
  flags: [
    {
    ...
    },
    {
      ...
      code: "BACKFLOW",
      title: "Backflow event detected",
      detail: "Reverse flow detected at meter MET-88234 on Mar 26 at 11:42 PM. Water flowing in the wrong direction can indicate a cross-connection or pressure differential issue in the distribution system.",
...
// CHARTS
// ─────────────────────────────────────────────────────────────────────────────
function BarChart({ data, height = 96, flagThreshold = 250, onBarClick }) {
    ...
    <div style={{ position: "relative" }}>
      {hovered !== null && (
        <div style={{
          ...
          {data[hovered].gallons > flagThreshold && " ⚠ spike"}
        </div>
      )}
          ...
          return (
            <div key={i} style={{ flex: 1 }}>
              <div
                ...
                style={{
                  width: "100%", height: h,
                  background: flagged ? "#EF4444" : hovered === i ? "#1A6A9A" : "var(--blue)",
    ...
    <div style={{ position: "relative" }}>
      {hovered !== null && (
        <div style={{
          ...
          {data[hovered].month} · {data[hovered].gallons.toLocaleString()} gal
        </div>
      )}
          ...
          return (
            <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
              <div
                ...
                style={{
                  width: "100%", height: h,
                  background: isCurrent ? "var(--blue)" : hovered === i ? "#8AA4BF" : "var(--surface-3)",
...
function HourChart({ data, onBarClick }) {
    ...
    <div style={{ position: "relative" }}>
      {hovered !== null && (
        <div style={{
          ...
          {hourLabels[hovered]} · {data[hovered]} gal
          {hovered < 4 && data[hovered] > 0 ? " · overnight" : ""}
        </div>
      )}
      <div style={{ display: "flex", alignItems: "flex-end", gap: 2, height: 48 }}>
        {data.map((v, i) => (
          <div
            ...
            onMouseEnter={() => setHovered(i)}
            onMouseLeave={() => setHovered(null)}
            onClick={() => onBarClick && v > 0 && onBarClick(i)}
            style={{
              flex: 1,
              height: Math.max(2, Math.round((v / max) * 46)),
              background: v === 0 ? "var(--surface-3)" : i < 4 ? "#EF4444" : hovered === i ? "#1A6A9A" : "var(--blue)",
              borderRadius: "2px 2px 0 0",
              opacity: hovered === null || hovered === i ? 1 : 0.55,
              transition: "background 0.12s, opacity 0.15s",
              cursor: onBarClick && v > 0 ? "pointer" : "default",
            }}
          />
        ))}
      </div>
...
// ── EXPANDED HOUR CHART MODAL ──
function HourExpandModal({ hour, data, onClose }) {
  const hourLabels = ["12:00 AM","1:00 AM","2:00 AM","3:00 AM","4:00 AM","5:00 AM","6:00 AM","7:00 AM","8:00 AM","9:00 AM","10:00 AM","11:00 AM","12:00 PM","1:00 PM","2:00 PM","3:00 PM","4:00 PM","5:00 PM","6:00 PM","7:00 PM","8:00 PM","9:00 PM","10:00 PM","11:00 PM"];
  const max = Math.max(...data) || 1;
  const total = data.reduce((s, v) => s + v, 0);
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(10,22,40,0.7)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
      onClick={onClose}>
      <div style={{ background: "var(--surface)", border: "1px solid var(--rule)", borderRadius: 16, padding: 24, width: "100%", maxWidth: 500, maxHeight: "90vh", overflowY: "auto" }}
        onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink)" }}>Hourly usage — Today</div>
            <div style={{ fontSize: 12, color: "var(--ink-3)", marginTop: 2 }}>Total: {total} gal · Meter MET-88234</div>
          </div>
          <button onClick={onClose} style={{ fontSize: 12, padding: "4px 12px", background: "var(--surface-2)", border: "1px solid var(--rule)", borderRadius: 6, cursor: "pointer", color: "var(--ink-3)" }}>✕ Close</button>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {data.map((v, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ fontSize: 11, color: "var(--ink-4)", width: 64, textAlign: "right", flexShrink: 0 }}>{hourLabels[i]}</div>
              <div style={{ flex: 1, height: 18, background: "var(--surface-2)", borderRadius: 4, overflow: "hidden", position: "relative" }}>
                <div style={{
                  ...
                  background: i < 4 && v > 0 ? "#EF4444" : i === hour ? "#F59E0B" : "var(--blue)",
                  transition: "width 0.4s ease",
                }}/>
              </div>
              <div style={{ fontSize: 12, fontWeight: v > 0 ? 600 : 400, color: v > 0 ? "var(--ink)" : "var(--ink-4)", width: 44, textAlign: "right", flexShrink: 0 }}>
                {v > 0 ? `${v} gal` : "—"}
              </div>
            </div>
          ))}
        </div>
        <div style={{ marginTop: 14, padding: "10px 14px", background: "var(--surface-2)", borderRadius: 8, border: "1px solid var(--rule)", fontSize: 12, color: "var(--ink-3)" }}>
          {data[0] > 0 || data[1] > 0 || data[2] > 0 || data[3] > 0
            ? "⚠ Overnight usage detected (12am–4am) — possible continuous flow event"
            : "Peak usage during morning hours (6am–9am) — consistent with normal household pattern"}
        </div>
      </div>
    </div>
  );
...
// ── EXPANDED DAY CHART MODAL ──
function DayExpandModal({ day, data, onClose }) {
  const max = Math.max(...data.map(d => d.gal));
  const total = data.reduce((s, d) => s + d.gal, 0);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(10,22,40,0.7)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--rule)", borderRadius: 16, padding: 24, width: "100%", maxWidth: 500, maxHeight: "90vh", overflowY: "auto" }}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink)" }}>Daily usage — March 2026</div>
            <div style={{ fontSize: 12, color: "var(--ink-3)", marginTop: 2 }}>Total: {total.toLocaleString()} gal · Avg: {avg} gal/day</div>
          </div>
          <button onClick={onClose} style={{ fontSize: 12, padding: "4px 12px", background: "var(--surface-2)", border: "1px solid var(--rule)", borderRadius: 6, cursor: "pointer", color: "var(--ink-3)" }}>✕ Close</button>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {data.map((d, i) => {
            const isSpike = d.gal > 250;
            const isSelected = d.day === day;
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ fontSize: 11, color: "var(--ink-4)", width: 44, textAlign: "right", flexShrink: 0 }}>{d.day}</div>
                <div style={{ flex: 1, height: 18, background: "var(--surface-2)", borderRadius: 4, overflow: "hidden" }}>
                  <div style={{
                    height: "100%", borderRadius: 4,
                    width: `${Math.round((d.gal / max) * 100)}%`,
                    background: isSpike ? "#EF4444" : isSelected ? "#F59E0B" : "var(--blue)",
                    transition: "width 0.4s ease",
                  }}/>
                </div>
                <div style={{ fontSize: 12, fontWeight: isSpike ? 700 : 400, color: isSpike ? "#991B1B" : "var(--ink)", width: 52, textAlign: "right", flexShrink: 0 }}>
                  {d.gal} gal
                </div>
                {isSpike && <div style={{ fontSize: 10, color: "#EF4444", flexShrink: 0 }}>⚠</div>}
              </div>
            );
          })}
        </div>
        <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <div style={{ padding: "10px 12px", background: "#FEF2F2", borderRadius: 8, border: "1px solid #FCA5A5", fontSize: 11, color: "#991B1B" }}>
            <strong>Highest:</strong> Mar 17 · 310 gal (leak detected)
          </div>
          <div style={{ padding: "10px 12px", background: "var(--surface-2)", borderRadius: 8, border: "1px solid var(--rule)", fontSize: 11, color: "var(--ink-3)" }}>
            <strong style={{ color: "var(--ink)" }}>Lowest:</strong> Mar 1 · 138 gal
          </div>
        </div>
      </div>
    </div>
...
const TIMELINE_DATA = {
  annual: [
    {l:"Apr '25",g:3820},{l:"May '25",g:4210},{l:"Jun '25",g:5640},
    {l:"Jul '25",g:6180},{l:"Aug '25",g:5920},{l:"Sep '25",g:4870},
    {l:"Oct '25",g:3760},{l:"Nov '25",g:3210},{l:"Dec '25",g:3050},
    {l:"Jan '26",g:2980},{l:"Feb '26",g:3240},{l:"Mar '26",g:4820},
  ],
  monthly: {
    "Apr '25":[142,135,158,151,138,144,149,162,168,145,155,148,152,178,158,171,163,148,155,138,161,189,154,167,160,145,149,138,152,145],
    "May '25":[148,155,162,171,158,165,178,185,172,160,154,162,148,165,178,168,155,162,175,165,172,158,168,175,162,155,162,171,158,165,148],
    "Jun '25":[168,175,188,195,201,188,195,208,215,201,188,182,195,188,201,215,205,192,198,211,201,208,195,205,211,198,192,198,207,195],
    "Jul '25":[195,202,215,222,228,215,222,235,241,228,215,208,221,215,228,241,231,218,225,238,228,235,221,231,238,225,218,225,234,221,228],
    "Aug '25":[188,195,208,215,221,208,215,228,235,221,208,202,215,208,221,235,225,212,218,231,221,228,215,225,231,218,212,218,227,215,221],
    "Sep '25":[162,168,181,188,194,181,188,201,207,194,181,175,188,181,194,207,197,184,191,204,194,200,187,197,203,191,184,191,200,187],
    "Oct '25":[142,148,161,168,174,161,168,181,187,174,161,155,168,161,174,187,177,164,171,184,174,181,167,177,183,171,164,171,180,167,174],
    "Nov '25":[121,127,140,147,153,140,147,160,166,153,140,134,147,140,153,166,156,143,150,163,153,160,146,156,162,150,143,150,159,146],
    "Dec '25":[115,121,134,141,147,134,141,154,160,147,134,128,141,134,147,160,150,137,144,157,147,154,140,150,156,144,137,144,153,140,147],
    "Jan '26":[112,118,131,138,144,131,138,151,157,144,131,125,138,131,144,157,147,134,141,154,144,151,137,147,153,141,134,141,150,137,144],
    "Feb '26":[125,131,144,151,157,144,151,164,170,157,144,138,151,144,157,170,160,147,154,167,157,164,150,160,166,154,147,154,163],
    "Mar '26":[142,135,138,151,158,144,149,162,168,221,155,148,152,145,158,171,310,163,148,155,172,161,189,154,167,160,145,149,138],
  },
  hourly: {
    "17":[0,0,0,18,22,18,12,28,34,22,14,18,16,12,8,14,18,24,22,16,10,8,6,4],
    def:[2,2,2,2,0,0,8,18,22,14,9,6,4,11,18,22,16,8,12,20,14,6,2,0],
  },
};

const HL_LABELS=["12a","1a","2a","3a","4a","5a","6a","7a","8a","9a","10a","11a","12p","1p","2p","3p","4p","5p","6p","7p","8p","9p","10p","11p"];

function UsageTimeline() {
  const [view, setView]         = useState("annual");
  const [selMonth, setSelMonth] = useState("Mar '26");
  const [selDay, setSelDay]     = useState(null);
  const [hovered, setHovered]   = useState(null);

  const getData = () => {
    if (view === "annual")  return TIMELINE_DATA.annual;
    if (view === "monthly") return (TIMELINE_DATA.monthly[selMonth] || []).map((g, i) => ({ l: String(i + 1), g }));
    if (view === "daily")   return (TIMELINE_DATA.hourly[selDay] || TIMELINE_DATA.hourly.def).map((g, i) => ({ l: HL_LABELS[i], g }));
    return [];
  };

  const data  = getData();
  const vals  = data.map(d => d.g);
  ...
  const isSpike = (g, i) => {
    if (view === "annual")  return g > 5500;
    if (view === "monthly") return g > 250;
    if (view === "daily")   return i >= 3 && i <= 5 && g > 15;
    return false;
  };

  const fmtLabel = g => view === "annual" ? (g / 1000).toFixed(1) + "k" : g > 0 ? String(g) : "";
  const unit     = view === "annual" ? "gal/mo" : view === "monthly" ? "gal/day" : "gal/hr";
  const showEvery = view === "annual" ? 1 : view === "monthly" ? 5 : 3;
  const bc = view === "annual" ? "Apr 2025 – Mar 2026"
           : view === "monthly" ? `${selMonth} — daily breakdown`
           : `${selMonth} · Day ${selDay} — hourly`;

  const handleClick = (d, i) => {
    if (view === "annual")  { setSelMonth(d.l); setView("monthly"); setHovered(null); }
    if (view === "monthly") { setSelDay(d.l);   setView("daily");   setHovered(null); }
  };

  const goBack = () => { setView(view === "daily" ? "monthly" : "annual"); setHovered(null); };

  return (
    <Card style={{ marginBottom: 16 }}>
      <CardHeader title="Usage timeline" sub={bc} />
      <div style={{ padding: "0 20px 16px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
          <div style={{ display: "flex", background: "var(--surface-2)", border: "1px solid var(--rule)", borderRadius: 8, padding: 3, gap: 2 }}>
            {["annual","monthly","daily"].map(v => (
              <button key={v} onClick={() => { setView(v); setHovered(null); }} style={{
                background: view === v ? "var(--blue)" : "transparent", border: "none",
                borderRadius: 6, padding: "5px 14px", fontSize: 12,
                fontWeight: view === v ? 600 : 400, color: view === v ? "#fff" : "var(--ink-3)", cursor: "pointer",
              }}>{v.charAt(0).toUpperCase() + v.slice(1)}</button>
            ))}
          </div>
          {view === "monthly" && (
            <select value={selMonth} onChange={e => { setSelMonth(e.target.value); setHovered(null); }} style={{
              background: "var(--surface-2)", border: "1px solid var(--rule)", borderRadius: 8,
              padding: "6px 12px", fontSize: 12, color: "var(--ink-2)",
            }}>
              {TIMELINE_DATA.annual.map(m => <option key={m.l} value={m.l}>{m.l}</option>)}
            </select>
          )}
          {view !== "annual" && (
            <button onClick={goBack} style={{
              background: "var(--surface-2)", border: "1px solid var(--rule)", borderRadius: 8,
              padding: "6px 12px", fontSize: 12, color: "var(--ink-3)", cursor: "pointer",
            }}>← Back</button>
          )}
          {view !== "daily" && (
            <span style={{ fontSize: 11, color: "var(--blue)", fontWeight: 600, marginLeft: "auto" }}>Click any bar to drill down →</span>
          )}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
          {[
            { label: view === "annual" ? "12-month total" : view === "monthly" ? "Monthly total" : "Daily total", val: total.toLocaleString(), unit: "gallons" },
            { label: view === "annual" ? "Monthly avg" : view === "monthly" ? "Daily avg" : "Hourly avg", val: avg.toLocaleString(), unit },
            { label: "Peak", val: max.toLocaleString(), unit },
          ].map((k, i) => (
            <div key={i} style={{ background: "var(--surface-2)", border: "1px solid var(--rule)", borderRadius: 8, padding: "10px 12px" }}>
              <div style={{ fontSize: 9, fontWeight: 600, color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 3 }}>{k.label}</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: "var(--ink)", lineHeight: 1 }}>{k.val}</div>
              <div style={{ fontSize: 10, color: "var(--ink-4)", marginTop: 2 }}>{k.unit}</div>
            </div>
          ))}
        </div>
        <div>
          <div style={{ display: "flex", alignItems: "flex-end", gap: view === "annual" ? 6 : 3, height: CHART_H, borderBottom: "1px solid var(--rule)" }}>
            {data.map((d, i) => {
              const barH = Math.max(3, Math.round((d.g / max) * (CHART_H - 20)));
              const spike = isSpike(d.g, i);
              const col = spike ? "#EF4444" : hovered === i ? "#1A6A9A" : "var(--blue)";
              return (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", height: "100%" }}>
                  <div style={{ fontSize: 8, color: "var(--ink-4)", marginBottom: 2, whiteSpace: "nowrap" }}>{fmtLabel(d.g)}</div>
                  <div
                    onMouseEnter={() => setHovered(i)}
                    onMouseLeave={() => setHovered(null)}
                    onClick={() => view !== "daily" && handleClick(d, i)}
                    style={{
                      width: "100%", height: barH, background: col,
                      borderRadius: "3px 3px 0 0",
                      opacity: hovered === null || hovered === i ? 1 : 0.4,
                      cursor: view !== "daily" ? "pointer" : "default",
          ...
          <div style={{ display: "flex", gap: view === "annual" ? 6 : 3, marginTop: 4 }}>
            {data.map((d, i) => (
              <div key={i} style={{ flex: 1, textAlign: "center", fontSize: 8, color: "var(--ink-4)", overflow: "hidden", whiteSpace: "nowrap" }}>
                {(i % showEvery === 0 || i === data.length - 1) ? d.l : ""}
              </div>
            ))}
          </div>
        </div>
        <div style={{ display: "flex", gap: 14, marginTop: 12, alignItems: "center" }}>
          <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
            ...
            <span style={{ fontSize: 10, color: "var(--ink-4)" }}>Normal usage</span>
          </div>
          <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
            <div style={{ width: 10, height: 10, borderRadius: 2, background: "#EF4444" }}/>
            <span style={{ fontSize: 10, color: "var(--ink-4)" }}>Spike / anomaly</span>
          </div>
        </div>
      </div>
    </Card>
...
function Overview({ onNav }) {
  ...
  const dailyData = [
    { day: "Mar 1",  gal: 142 }, { day: "Mar 3",  gal: 138 }, { day: "Mar 5",  gal: 158 },
    { day: "Mar 7",  gal: 144 }, { day: "Mar 9",  gal: 162 }, { day: "Mar 10", gal: 221 },
    { day: "Mar 12", gal: 149 }, { day: "Mar 14", gal: 155 }, { day: "Mar 15", gal: 145 },
    { day: "Mar 17", gal: 310 }, { day: "Mar 19", gal: 171 }, { day: "Mar 20", gal: 163 },
    { day: "Mar 22", gal: 148 }, { day: "Mar 24", gal: 172 }, { day: "Mar 26", gal: 189 },
    { day: "Mar 27", gal: 160 },
  ];
  const maxDaily = Math.max(...dailyData.map(d => d.gal));

  return (
    <div className="fade-up">

      {hourModal !== null && <HourExpandModal hour={hourModal} data={DATA.hourly} onClose={() => setHourModal(null)} />}
      {dayModal !== null && <DayExpandModal day={dayModal} data={dailyData} onClose={() => setDayModal(null)} />}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 14 }}>
        ...
        {/* TODAY */}
        <div>
          <Card style={{ padding: "14px 16px", cursor: "pointer", border: expanded === "today" ? "1px solid var(--blue-mid)" : undefined }}
            onClick={() => toggle("today")}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--ink-4)" }}>Today</div>
              <div style={{ fontSize: 10, color: "var(--blue)", fontWeight: 600 }}>{expanded === "today" ? "▲ Less" : "▼ More"}</div>
            </div>
            <HourChart data={DATA.hourly} onBarClick={i => setHourModal(i)} />
            <div style={{ fontSize: 9, color: "var(--blue)", marginTop: 4, textAlign: "center", letterSpacing: "0.03em" }}>Click any bar for details</div>
          ...
          {expanded === "today" && (
            <Card style={{ marginTop: 6, padding: "14px 16px", border: "1px solid var(--blue-mid)", background: "var(--blue-light)" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--blue)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Hourly breakdown</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {DATA.hourly.map((v, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ fontSize: 9, color: "var(--ink-4)", width: 28, textAlign: "right", flexShrink: 0 }}>{hourLabels[i]}</div>
                    <div style={{ flex: 1, height: 10, background: "var(--surface-2)", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{
                        height: "100%", borderRadius: 3,
                        width: `${Math.round((v / Math.max(...DATA.hourly)) * 100)}%`,
                        background: i === peakHour ? "#EF4444" : v > 15 ? "var(--blue)" : "var(--blue-mid)",
                      }}/>
                    </div>
                    <div style={{ fontSize: 9, color: v > 0 ? "var(--ink-2)" : "var(--ink-4)", width: 28, flexShrink: 0, fontWeight: v === Math.max(...DATA.hourly) ? 700 : 400 }}>{v > 0 ? `${v}g` : "—"}</div>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 10, padding: "8px 10px", background: "rgba(255,255,255,0.6)", borderRadius: 6, border: "1px solid var(--blue-mid)", fontSize: 11, color: "var(--ink-3)" }}>
                Peak usage at <strong style={{ color: "var(--ink)" }}>{hourLabels[peakHour]}</strong> — {Math.max(...DATA.hourly)} gal · Typical morning routine pattern
              </div>
            </Card>
          )}
        </div>
        ...
        {/* THIS MONTH */}
        <div>
          <Card style={{ padding: "14px 16px", cursor: "pointer", border: expanded === "month" ? "1px solid var(--blue-mid)" : undefined }}
            onClick={() => toggle("month")}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--ink-4)" }}>This month</div>
              <div style={{ fontSize: 10, color: "var(--blue)", fontWeight: 600 }}>{expanded === "month" ? "▲ Less" : "▼ More"}</div>
            </div>
            <BarChart data={DATA.reads} height={60} onBarClick={i => setDayModal(DATA.reads[i].date)} />
            <div style={{ fontSize: 9, color: "var(--blue)", marginTop: 4, textAlign: "center", letterSpacing: "0.03em" }}>Click any bar for details</div>
            <div style={{ marginTop: 8, borderTop: "1px solid var(--rule)", paddingTop: 8 }}>
              <div style={{ fontSize: 18, fontWeight: 700, color: "var(--ink)", lineHeight: 1 }}>
          ...
          {expanded === "month" && (
            <Card style={{ marginTop: 6, padding: "14px 16px", border: "1px solid var(--blue-mid)", background: "var(--blue-light)" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--blue)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Daily breakdown — March 2026</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {dailyData.map((d, i) => {
                  const isSpike = d.gal > 250;
                  const isLeak  = d.day === "Mar 17";
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ fontSize: 9, color: "var(--ink-4)", width: 36, textAlign: "right", flexShrink: 0 }}>{d.day}</div>
                      <div style={{ flex: 1, height: 12, background: "var(--surface-2)", borderRadius: 3, overflow: "hidden" }}>
                        <div style={{
                          height: "100%", borderRadius: 3,
                          width: `${Math.round((d.gal / maxDaily) * 100)}%`,
                          background: isLeak ? "#EF4444" : isSpike ? "#F59E0B" : "var(--blue)",
                        }}/>
                      </div>
                      <div style={{ fontSize: 9, width: 36, flexShrink: 0, fontWeight: isSpike ? 700 : 400, color: isLeak ? "#991B1B" : "var(--ink-2)" }}>{d.gal}g</div>
                      {isLeak && <div style={{ fontSize: 9, color: "#991B1B", flexShrink: 0 }}>⚠ leak</div>}
                    </div>
                  );
                })}
              </div>
              <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                <div style={{ padding: "8px 10px", background: "rgba(255,255,255,0.6)", borderRadius: 6, border: "1px solid var(--blue-mid)", fontSize: 11, color: "var(--ink-3)" }}>
                  <strong style={{ color: "var(--ink)" }}>Highest day:</strong> Mar 17 · 310 gal <span style={{ color: "#991B1B" }}>(leak detected)</span>
                </div>
                <div style={{ padding: "8px 10px", background: "rgba(255,255,255,0.6)", borderRadius: 6, border: "1px solid var(--blue-mid)", fontSize: 11, color: "var(--ink-3)" }}>
                  <strong style={{ color: "var(--ink)" }}>Lowest day:</strong> Mar 1 · 138 gal
                </div>
              </div>
            </Card>
          )}
        ...
        {/* 6 MONTHS */}
        <div>
          <Card style={{ padding: "14px 16px", cursor: "pointer", border: expanded === "history" ? "1px solid var(--blue-mid)" : undefined }}
            onClick={() => toggle("history")}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--ink-4)" }}>6 months</div>
              <div style={{ fontSize: 10, color: "var(--blue)", fontWeight: 600 }}>{expanded === "history" ? "▲ Less" : "▼ More"}</div>
            </div>
          ...
          {expanded === "history" && (
            <Card style={{ marginTop: 6, padding: "14px 16px", border: "1px solid var(--blue-mid)", background: "var(--blue-light)" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--blue)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Month-by-month detail</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 0, borderRadius: 8, overflow: "hidden", border: "1px solid var(--rule)" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", padding: "6px 10px", background: "var(--surface-2)", borderBottom: "1px solid var(--rule)" }}>
                  {["Month", "Usage", "Daily avg", "vs. prior"].map(h => (
                    <div key={h} style={{ fontSize: 9, fontWeight: 700, color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.05em" }}>{h}</div>
                  ))}
                </div>
                {DATA.history.map((h, i) => {
                  const prior = i > 0 ? DATA.history[i - 1].gallons : null;
                  const chg = prior ? Math.round(((h.gallons - prior) / prior) * 100) : null;
                  const isCurrent = i === DATA.history.length - 1;
                  return (
                    <div key={i} style={{
                      display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr",
                      padding: "8px 10px", background: isCurrent ? "rgba(255,255,255,0.5)" : "var(--surface)",
                      borderBottom: i < DATA.history.length - 1 ? "1px solid var(--rule)" : "none",
                    }}>
                      <div style={{ fontSize: 11, fontWeight: isCurrent ? 700 : 400, color: isCurrent ? "var(--blue)" : "var(--ink-2)" }}>{h.month}</div>
                      <div style={{ fontSize: 11, fontWeight: 600, color: "var(--ink)" }}>{h.gallons.toLocaleString()}</div>
                      <div style={{ fontSize: 11, color: "var(--ink-3)" }}>{Math.round(h.gallons / 30)} gal</div>
                      <div style={{ fontSize: 11, color: chg === null ? "var(--ink-4)" : chg > 0 ? "#991B1B" : "#0F7B52", fontWeight: 600 }}>
                        {chg === null ? "—" : `${chg > 0 ? "+" : ""}${chg}%`}
                      </div>
                    </div>
                  );
              ...
              <div style={{ marginTop: 10, padding: "8px 10px", background: "rgba(255,255,255,0.6)", borderRadius: 6, border: "1px solid var(--blue-mid)", fontSize: 11, color: "var(--ink-3)" }}>
                Usage has increased <strong style={{ color: "#991B1B" }}>28% over 6 months</strong> — Oct '25 baseline was {DATA.history[0].gallons.toLocaleString()} gal
              </div>
            </Card>
          )}
        </div>
...
function Usage() {
  ...
  return (
    <div className="fade-up">
      ...
      <Card style={{ marginBottom: 16 }}>
        <CardHeader title="Annual water allotment" sub={DATA.allotment.periodLabel} />
        <div style={{ padding: "16px 20px" }}>
          {(() => {
            const { annualGal, usedGal, monthsRemaining } = DATA.allotment;
            const remainingGal = annualGal - usedGal;
            const usedPct = Math.round((usedGal / annualGal) * 100);
            const projectedTotal = Math.round(usedGal + (usedGal / 6) * monthsRemaining);
            const onTrack = projectedTotal <= annualGal;
            const avgMonthlyAllowed = Math.round(remainingGal / monthsRemaining);
            return (
              <>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
                  <div style={{ background: "var(--surface-2)", border: "1px solid var(--rule)", borderRadius: 10, padding: "12px 14px" }}>
                    <div style={{ fontSize: 10, fontWeight: 600, color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>Used so far</div>
                    <div style={{ fontSize: 20, fontWeight: 800, color: "var(--ink)", lineHeight: 1 }}>{usedGal.toLocaleString()}</div>
                    <div style={{ fontSize: 11, color: "var(--ink-4)", marginTop: 3 }}>gal · {usedPct}% of annual</div>
                  </div>
                  <div style={{ background: "var(--surface-2)", border: "1px solid var(--rule)", borderRadius: 10, padding: "12px 14px" }}>
                    <div style={{ fontSize: 10, fontWeight: 600, color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>Remaining</div>
                    <div style={{ fontSize: 20, fontWeight: 800, color: "#0F7B52", lineHeight: 1 }}>{remainingGal.toLocaleString()}</div>
                    <div style={{ fontSize: 11, color: "var(--ink-4)", marginTop: 3 }}>gal · {monthsRemaining} months left</div>
                  </div>
                  <div style={{
                    background: onTrack ? "#ECFDF5" : "#FEF3C7",
                    border: `1px solid ${onTrack ? "#6EE7B7" : "#FCD34D"}`,
                    borderRadius: 10, padding: "12px 14px",
                  }}>
                    <div style={{ fontSize: 10, fontWeight: 600, color: "var(--ink-4)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>Projection</div>
                    <div style={{ fontSize: 20, fontWeight: 800, color: onTrack ? "#065F46" : "#92400E", lineHeight: 1 }}>{onTrack ? "On track" : "Over pace"}</div>
                    <div style={{ fontSize: 11, color: "var(--ink-4)", marginTop: 3 }}>Est. {projectedTotal.toLocaleString()} gal/yr</div>
                  </div>
                </div>
                <div style={{ marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: "var(--ink)" }}>Annual allotment progress</span>
                  <span style={{ fontSize: 12, color: "var(--ink-3)" }}>{usedGal.toLocaleString()} of {annualGal.toLocaleString()} gal</span>
                </div>
                <div style={{ height: 12, background: "var(--surface-2)", borderRadius: 6, overflow: "hidden", border: "1px solid var(--rule)", marginBottom: 8 }}>
                  <div style={{
                    height: "100%", borderRadius: 6,
                    width: `${usedPct}%`,
                    background: usedPct > 80 ? "#EF4444" : usedPct > 60 ? "#F59E0B" : "#0F7B52",
                    transition: "width 0.5s ease",
                  }}/>
                </div>
                <div style={{ padding: "10px 12px", background: "var(--surface-2)", borderRadius: 8, border: "1px solid var(--rule)", fontSize: 12, color: "var(--ink-3)" }}>
                  To stay within your allotment, aim for no more than <strong style={{ color: "var(--ink)" }}>{avgMonthlyAllowed.toLocaleString()} gal/month</strong> over the next {monthsRemaining} months.
                </div>
              </>
            );
          })()}
...
const FLAG_META = {
  ...
  BACKFLOW: {
    icon: "🔄",
    what: "Your meter detected water flowing in the reverse direction. Backflow can allow contaminants to enter the clean water supply and is considered a serious event.",
    likely: ["Back-siphonage from a drop in supply pressure", "Cross-connection with irrigation or pool systems", "Faulty or missing backflow prevention device"],
    action: "Contact your utility immediately. Do not use water until the issue is assessed. A licensed plumber should inspect and test your backflow prevention assembly.",
  },
};
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
            ...
            <div>
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4, flexWrap: "wrap" }}>
                <span style={{ fontSize: 14, fontWeight: 700, color: "#065F46" }}>Leak resolved — continuous flow stopped</span>
                <span style={{ fontSize: 10, fontWeight: 700, background: "#D1FAE5", color: "#065F46", border: "1px solid #6EE7B7", borderRadius: 99, padding: "1px 8px", letterSpacing: "0.04em" }}>Resolved</span>
              </div>
              <div style={{ fontSize: 13, color: "#047857", lineHeight: 1.5 }}>
                Good news — your meter is no longer detecting continuous flow. The leak that was flagged on Mar 27 at 3:14 AM appears to have been resolved. No further action is needed unless flow resumes.
              </div>
              <div style={{ fontSize: 11, color: "#6EE7B7", marginTop: 5, fontWeight: 600 }}>Today, 8:42 AM · Sent via email to sarah.mitchell@email.com</div>
            </div>
          </div>
          <button
            onClick={() => setResolvedDismissed(true)}
            style={{ fontSize: 11, padding: "4px 10px", background: "transparent", border: "1px solid rgba(0,0,0,0.12)", color: "#047857", flexShrink: 0, borderRadius: 6, cursor: "pointer" }}
          >Dismiss</button>
        </div>
      )}
      ...
      <Card style={{ marginTop: visible.length > 0 ? 8 : 0 }}>
        ...
        <div>
          {[
            ...
            { label: "Backflow event",  desc: "Reverse flow detected. May indicate a cross-connection risk — contact your utility." },
...
function Notifications() {
  ...
  return (
    <div className="fade-up">
      ...
      {/* ── BOIL ORDER / PUBLIC HEALTH DIRECTIVE ── */}
      <div style={{
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: "#9F1239" }}>Public Health Directive — Boil Water Advisory</span>
              <span style={{ fontSize: 10, fontWeight: 700, background: "#9F1239", color: "#fff", padding: "2px 8px", borderRadius: 99, letterSpacing: "0.04em" }}>Active</span>
            </div>
            <div style={{ fontSize: 13, color: "#BE123C", lineHeight: 1.6, marginBottom: 8 }}>
              A boil water advisory is currently in effect for your service area. All water used for drinking, cooking, brushing teeth, or making ice must be brought to a full rolling boil for at least <strong>1 minute</strong> before use.
            </div>
            <div style={{ fontSize: 12, color: "#9F1239", marginBottom: 10 }}>
              <strong>Issued:</strong> Mar 26, 2026 · <strong>Issued by:</strong> Mesa County Water District · <strong>Affected area:</strong> North District zones 4, 5 &amp; 6
            </div>
            <div style={{ background: "rgba(255,255,255,0.6)", border: "1px solid #FDA4AF", borderRadius: 8, padding: "10px 14px" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#9F1239", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>What you should do</div>
              {[
                "Boil all tap water for 1 full minute before drinking, cooking, or making ice",
                "Do not use tap water from refrigerator ice makers or water dispensers",
                "Bottled water is safe to use and available at designated distribution sites",
                "Boiling is not necessary for bathing, laundry, or toilet use",
              ].map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 12, color: "#BE123C", marginBottom: i < 3 ? 5 : 0, lineHeight: 1.5 }}>
                  <span style={{ flexShrink: 0, marginTop: 1 }}>·</span>{item}
                </div>
              ))}
            </div>
            <div style={{ fontSize: 11, color: "#FDA4AF", marginTop: 10, fontWeight: 600 }}>
              Sent via SMS &amp; Email · Mar 26, 7:00 AM · This notice will be updated when the advisory is lifted
            </div>
          </div>
        </div>
      </div>
      ...
      <Card style={{ marginBottom: 16 }}>
        <CardHeader title="Recent notifications" sub="Alerts sent to your contact channels" />
        <div>
          {[
            {
              icon: "🚨", bg: "#FFF1F2", border: "#FDA4AF", titleColor: "#9F1239",
              title: "Boil water advisory — public health directive",
              desc: "A boil water advisory is in effect for your area. Boil all water for 1 minute before use for drinking or cooking.",
              time: "Mar 26, 7:00 AM", via: "SMS & Email",
            },
            {
              icon: "✅", bg: "#ECFDF5", border: "#6EE7B7", titleColor: "#065F46",
              title: "Leak resolved — continuous flow stopped",
              desc: "Your meter is no longer detecting continuous flow. The leak flagged Mar 27 at 3:14 AM appears resolved.",
              time: "Today, 8:42 AM", via: "Email",
            },
            {
              icon: "💧", bg: "#FEF2F2", border: "#FCA5A5", titleColor: "#991B1B",
              title: "Leak detected — continuous flow at meter",
              desc: "Continuous flow of 1.5 GPM detected for 3 hours overnight. Possible running toilet or slow leak.",
              time: "Today, 3:14 AM", via: "Email",
            },
            {
              icon: "💳", bg: "var(--surface-2)", border: "var(--rule)", titleColor: "var(--ink)",
              title: "Bill ready — $42.18 due April 15",
              desc: "Your March 2026 bill is available. Payment is due by April 15, 2026.",
              time: "Apr 1, 9:00 AM", via: "Email",
            },
          ].map((n, i, a) => (
            <div key={i} style={{
              display: "flex", gap: 12, padding: "13px 20px", alignItems: "flex-start",
              borderBottom: i < a.length - 1 ? "1px solid var(--rule)" : "none",
              background: i === 0 ? n.bg : "transparent",
            }}>
              <span style={{ fontSize: 18, flexShrink: 0, marginTop: 1 }}>{n.icon}</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: n.titleColor, marginBottom: 2 }}>{n.title}</div>
                <div style={{ fontSize: 12, color: "var(--ink-3)", lineHeight: 1.5, marginBottom: 4 }}>{n.desc}</div>
                <div style={{ fontSize: 11, color: "var(--ink-4)" }}>{n.time} · via {n.via}</div>
              </div>
            </div>
          ))}
        </div>
...
function TabBar({ tab, setTab }) {
  ...
  const arrowBtn = (dir, enabled) => ({
    ...
    border: enabled ? "1px solid var(--rule)" : "none",
    borderRadius: 6, width: 28, height: 28,
    display: "flex", alignItems: "center", justifyContent: "center",
    cursor: enabled ? "pointer" : "default",
    color: enabled ? "var(--ink-2)" : "transparent",
    fontSize: 13, flexShrink: 0,
    transition: "background 0.15s, border-color 0.15s",
  });

  return (
    <div style={{ display: "flex", alignItems: "center", gap: 4, borderBottom: "1px solid var(--rule)", paddingBottom: 0 }}>
      <button style={arrowBtn(-1, canLeft)} onClick={() => canLeft && scroll(-1)} disabled={!canLeft}>‹</button>
      <div
        ref={ref}
        className="tab-bar"
        ...
        {TABS.map(t => (
          <button
            ...
            style={{
              ...
              whiteSpace: "nowrap", transition: "color 0.15s", cursor: "pointer",
            }}
          >{t.label}</button>
        ))}
      </div>
      <button style={arrowBtn(1, canRight)} onClick={() => canRight && scroll(1)} disabled={!canRight}>›</button>
    </div>
  );
}
